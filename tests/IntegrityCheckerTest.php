...
class IntegrityCheckerTest extends TestCase
{
    ...
    /**
     * @return \Magebit\AcpSpec\Api\Types\CardPaymentInstrumentDisplayInterface|null
     */
...
PHP);
        ...
        $this->assertSame(
            ['Magebit\\AcpSpec\\Api\\Types\\CardPaymentInstrumentDisplayInterface'],
            array_keys($dangling)
        );
    ...
    {
        $dir = $this->makeTempDir();
        $this->writeInterface($dir, 'Api\\Shopping', 'CheckoutInterface', '', ['Magebit\\AcpSpec\\Api\\Shopping\\FulfillmentInterface']);

        $dangling = (new IntegrityChecker())->findDanglingReferences($dir);

        $this->assertArrayHasKey('Magebit\\AcpSpec\\Api\\Shopping\\FulfillmentInterface', $dangling);
    }
...
PHP);
    ...
    public function testCrossNamespaceImportResolvesThroughTheUseMap(): void
    {
        ...
        $this->writeInterface(
            ...
            ['Magebit\\AcpSpec\\Api\\Types\\BuyerInterface']
        );
     ...
     * @param string $namespaceSuffix Namespace below Magebit\AcpSpec
    ...
    private function writeInterface(
        ...
        file_put_contents(
            $directory . '/' . $name . '.php',
            "<?php\n\ndeclare(strict_types=1);\n\nnamespace Magebit\\AcpSpec\\{$namespaceSuffix};\n\n"
            . $use . "\ninterface {$name}\n{\n{$body}\n}\n"
        );
    }
}

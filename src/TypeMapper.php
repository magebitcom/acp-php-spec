...
class TypeMapper
{
    ...
    /**
     * Interface names are disambiguated by the bundle namespace, so a $def name maps straight through.
     *
     * @param string $defName Definition name as written in $defs
     * @return string
     */
    public function definitionInterfaceName(string $defName): string
    {
        $name = $this->parser->toPascalCase($defName);
    ...
    private function resolveRefType(string $ref, string $currentFile): string
    {
        ...
        if ($defName === null) {
            $defName = $this->parser->getInterfaceName($schema, $target['file']);
        }

        return '\\' . $this->parser->getNamespaceFromPath($target['file'])
            . '\\' . $this->definitionInterfaceName($defName);
    }
